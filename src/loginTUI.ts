...
export async function loginTUI(user: User, screen: blessed.Widgets.Screen) {
    resetScreen(screen);
            ...
            const submit = blessed.button({
                ...
                inputForm.on('submit', async (data: any) => {
                        ...
                        if(name.trim().length === 0){
                            ...
                            if(userCheck){
                                message.display(`${name} logged in`, () => {
                                setUser(userCheck.name);
                                resetScreen(screen);
                                mainMenu(userCheck, screen);
                                });
                            } else {
                                message.display(`${name} not registered`, () => {
                                resetScreen(screen);
                                mainMenu(user, screen);
                            })
                            }
